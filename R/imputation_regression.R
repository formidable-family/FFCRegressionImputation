...
regression_imputation <- function(dataframe, method='lm', parallel=0, threshold=0.4,top_predictors=3, constructed=1,debug=0, test=0) {
	...
	if ("data.frame" %in% class(dataframe)) {
		...
		impute <- function(column, imputed_df, original_df) {
			...
			#for each column in imputed_df (processed in ways described above, not original input imputed_df)
		...
		if(test == 1) {
			...
			columnstorun <- subset(out_imputed, select=c('cm4hhinc', 'cf4hhinc', 'cm5adult', 'cm1bsex'))
			#columnstorun <- out_numeric[,1:4]
		} else {
			columnstorun <- out_imputed
		}

		#if parallelization option is set 
		if (parallel == 1) {
				final <- parallel::mclapply(colnames(columnstorun), function(x) impute(x, out_imputed, out_numeric), mc.cores=parallel::detectCores())
		...
		#run in sequence = off
				...
				final <- sapply(colnames(columnstorun), function(x) impute(x, out_imputed, out_numeric))
